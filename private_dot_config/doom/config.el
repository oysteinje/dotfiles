...
Example: 'Task 57855: docs(Update database hotel documentation)'
  -> 'docs/57855-update-database-hotel-documentation'"
  (interactive "sTask description: ")
  (let* ((task-num (and (string-match "\\(?:Task\\|task\\)\\s-*\\([0-9]+\\)" input)
                        (match-string 1 input)))
         (type-match (and (string-match "\\(docs\\|feat\\|fix\\)(" input)
                          (match-string 1 input)))
         (desc-match (and (string-match "(\\([^)]+\\))" input)
                          (match-string 1 input)))
         (prefix (or type-match "feat"))
         (description (or desc-match input))
         (slug (downcase
         ...
         (branch-name (concat prefix "/"
                              (if task-num (concat task-num "-") "")
                              slug)))
